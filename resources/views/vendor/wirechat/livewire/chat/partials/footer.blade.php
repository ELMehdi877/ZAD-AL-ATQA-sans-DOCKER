...
@endphp
<footer class="relative mt-auto h-auto shrink-0 border-t border-[#e8dfc4] bg-[#f8f5ee]/95 backdrop-blur">
    ...
    @else
        <div id="chat-footer" x-data="{ 'openEmojiPicker': false }"
            class="z-50 mx-auto flex w-full flex-col items-center gap-3 bg-transparent px-3 py-3 md:px-4 lg:px-6">

            {{-- Emoji section , we put it seperate to avoid interfering as overlay for form when opened --}}
            @if($hasEmojiPicker)
                {{-- If emoji picker is floading -wrap the emoji picke element into a teleport blade in order to allow proper
                render --}}
                {{-- --START-- TELEPORT --}}
                @if($floatingEmojiPicker) @teleport('body') @endif
                {{-- --END-- TELEPORT --}}
                <section wire:ignore x-cloak x-show="openEmojiPicker" @click.outside="openEmojiPicker=false"
                    @if($floatingEmojiPicker) x-anchor.top.offset.20="document.getElementById('emojipickerbutton')"
                    ...
                    x-transition:leave-end="opacity-0 translate-y-4 scale-90" dusk="floating-emojipicker" @else
                        x-transition:enter="transition  ease-out duration-180 transform"
                        x-transition:enter-start=" translate-y-full" x-transition:enter-end=" translate-y-0"
                        x-transition:leave="transition ease-in duration-180 transform" x-transition:leave-start=" translate-y-0"
                    x-transition:leave-end="translate-y-full" dusk="docked-emojipicker" @endif @class([
            ...
            <section class="z-50 mx-auto flex w-full flex-col items-center gap-2 py-0">

                {{-- Media preview section --}}
                <section x-show="$wire.media.length>0 ||$wire.files.length>0" x-cloak class="  flex flex-col w-full gap-3"
                        ...
                        <div x-data="attachments('media')">
                            ...
                            <section
                                ...
                                <label wire:loading.class="cursor-progress"
                                    ...
                                    <input wire:loading.attr="disabled" @change="handleFileSelect(event,{{ count($media) }})"
                                        type="file" multiple
                    ...
                    @if (count($files) > 0)
                        <section x-data="attachments('files')"
                            ...
                            {{-- TODO @if "( count($media)< $MAXFILES )" to hide upload button when maz files exceeded --}}
                                ...
                                <input wire:loading.attr="disabled" @change="handleFileSelect(event,{{ count($files) }})"
                                    type="file" multiple
                ...
                @if ($replyMessage != null)
                    <section class="w-full rounded-[20px] border border-[#eadfc6] bg-[#faf6ed] px-4 py-2.5 shadow-sm">
                        <div class="flex items-center justify-between text-[#4e4738] dark:text-white">
                            ...
                            <button wire:loading.attr="disabled" wire:click="removeReply()" class="text-[#7a705c] disabled:cursor-progress">
                        ...
                        <p class="mt-1 max-w-md truncate text-sm text-[#8e846f] dark:text-gray-200">
                ...
                <form x-data="{
                    ...
                    class="col-span-12 flex w-full min-h-[64px] items-end gap-3 rounded-[28px] border border-[#e8dfc4] bg-[#faf6ed] px-5 py-2.5 shadow-[0_2px_10px_rgba(0,0,0,0.04)]">
                    @csrf
                    ...
                    @if($hasEmojiPicker)
                        {{-- Emoji Triggger icon --}}
                        <div class="hidden w-11 max-w-fit items-center sm:flex">
                            <button wire:loading.attr="disabled" type="button" dusk="emoji-trigger-button"
                                x-on:keydown.escape.stop=" openEmojiPicker=false" @click="openEmojiPicker = ! openEmojiPicker"
                                id="emojipickerbutton"
                                class="inline-flex h-10 w-10 cursor-pointer items-center justify-center rounded-full bg-[#f1ead8] text-[#7c6c45] transition-transform hover:scale-105 hover:bg-[#e8dfc4] disabled:cursor-progress dark:border-gray-700">
                                <svg x-bind:style="openEmojiPicker && { color: 'var(--wc-brand-primary)' }" viewBox="0 0 24 24"
                                    height="24" width="24" preserveAspectRatio="xMidYMid meet"
                                    class="h-6 w-6 text-current dark:text-gray-300 srtoke-[1.3] dark:stroke-[1.2]"
                    ...
                    @if (count($this->media) == 0 && count($this->files) == 0 && $this->panel()->hasAttachments())
                        <x-wirechat::popover position="top" popoverOffset="70">
                            ...
                            <x-slot name="trigger" wire:loading.attr="disabled">
                                <span dusk="upload-trigger-button" class="inline-flex h-10 w-10 items-center justify-center rounded-full bg-[#f1ead8] text-[#7c6c45] transition-colors hover:bg-[#e8dfc4]">

                                    {{-- <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="w-7 h-7 dark:text-white/90">
                                    ...
                                    {{-- <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        ...
                                        class="size-6 w-7 h-7 text-gray-600 dark:text-white/90">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="m18.375 12.739-7.693 7.693a4.5 4.5 0 0 1-6.364-6.364l10.94-10.94A3 3 0 1 1 19.5 7.372L8.552 18.32m.009-.01-.01.01m5.699-9.941-7.81 7.81a1.5 1.5 0 0 0 2.112 2.13" />
                                    </svg> --}}
                                    <svg class="ai ai-Attach h-6 w-6 text-current dark:text-white/60"
                            ...
                            <div class="grid gap-2 w-full ">
                                ...
                                @if ($this->panel()->hasFileAttachments())
                                    <label wire:loading.class="cursor-progress" x-data="attachments('files')"
                                        ...
                                        <div
                                            ...
                                            <span>
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                                    style="color: var(--wc-brand-primary);" class="bi bi-folder-fill w-6 h-6"
                                                    viewBox="0 0 16 16">
                                                    <path
                                ...
                                @if ($this->panel()->hasMediaAttachments())
                                    <label wire:loading.class="cursor-progress" x-data="attachments('media')"
                                        ...
                                        <input dusk="media-upload-input" wire:loading.attr="disabled" wire:target="sendMessage"
                                        ...
                                        <div
                                            ...
                                            <span class="">
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                                                    class="w-6 h-6" style="color: var(--wc-brand-primary);">
                                                    <path fill-rule="evenodd"
                                                        d="M1.5 6a2.25 2.25 0 0 1 2.25-2.25h16.5A2.25 2.25 0 0 1 22.5 6v12a2.25 2.25 0 0 1-2.25 2.25H3.75A2.25 2.25 0 0 1 1.5 18V6ZM3 16.06V18c0 .414.336.75.75.75h16.5A.75.75 0 0 0 21 18v-1.94l-2.69-2.689a1.5 1.5 0 0 0-2.12 0l-.88.879.97.97a.75.75 0 1 1-1.06 1.06l-5.16-5.159a1.5 1.5 0 0 0-2.12 0L3 16.061Zm10.125-7.81a1.125 1.125 0 1 1 2.25 0 1.125 1.125 0 0 1-2.25 0Z"
                                                        clip-rule="evenodd" />
                                                </svg>
                    ...
                    <div @class(['flex w-full items-end gap-3'])>
                        <textarea @focus-input-field.window="$el.focus()" autocomplete="off" x-model='body' x-ref="body"
                            wire:loading.delay.longest.attr="disabled" wire:target="sendMessage" id="chat-input-field"
                            autofocus type="text" name="message"
                            placeholder="{{ __('wirechat::chat.inputs.message.placeholder') }}" maxlength="1700" rows="1"
                            ...
                            @keyup.enter.prevent="$event.shiftKey ? null : (((body && body?.trim().length > 0) || ($wire.media && $wire.media.length > 0) || ($wire.files && $wire.files.length > 0)) ? $wire.sendMessage() : null)"
                            class="wc-textarea flex h-auto max-h-32 w-full grow resize-none rounded-2xl border-0 bg-transparent px-1 py-2.5 text-[15px] text-[#24303b] outline-0 placeholder:text-[#9f9784] focus:border-0 focus:outline-hidden focus:ring-0 hover:ring-0 disabled:cursor-progress dark:bg-inherit dark:text-white overflow-y-auto scrollbar-hide"
                            x-init="
                                  @if($hasEmojiPicker)
                                    document.querySelector('emoji-picker')
                                        .addEventListener('emoji-click', event => {
                    ...
                    <div x-cloak @class(['flex min-w-max items-center justify-end gap-2'])>
                        ...
                        <div class="flex items-center">
                            <button
                                ...
                                wire:loading.attr="disabled" wire:target="sendMessage" type="submit" id="sendMessageButton"
                                aria-label="Envoyer"
                                class="flex h-11 shrink-0 cursor-pointer items-center justify-center gap-2 rounded-2xl bg-[#c69b3d] px-4 text-[15px] font-semibold text-[#14311c] shadow-sm transition-all hover:bg-[#b89237] disabled:cursor-progress disabled:opacity-50 active:scale-90">
                                <span>Envoyer</span>
                                <svg class="h-4 w-4 transform rotate-45 -translate-x-[1px]"
                        ...
                        @if($this->panel()->hasHeart())
                            ...
                            <button x-show="!((body?.trim()?.length>0) || $wire.media.length > 0 || $wire.files.length > 0 )"
                                ...
                                dusk="heart-button" type="button"
                                class="group inline-flex h-10 w-10 cursor-pointer items-center justify-center rounded-full bg-[#f1ead8] text-[#7c6c45] transition-transform hover:scale-105 disabled:cursor-progress">
