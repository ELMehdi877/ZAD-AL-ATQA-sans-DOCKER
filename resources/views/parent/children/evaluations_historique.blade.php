...
@section('content')
    <header class="mb-6 rounded-2xl bg-gradient-to-r from-[#04371f] to-[#04371f]/80 p-6 text-white shadow-xl flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold">Historique des evaluations de {{ $student->user->nom ?? '-' }} {{ $student->user->prenom ?? '' }}</h2>
            <p class="mt-2 text-sm text-slate-200">Toutes les evaluations, groupees par date.</p>
        </div>
        <a href="{{ route('parent.children', $student->id) }}" class="rounded-lg bg-white/20 px-4 py-2 text-sm font-medium text-white hover:bg-white/30 backdrop-blur-sm transition">Retour</a>
    </header>

        <div class="mb-6 rounded-lg bg-white p-4 shadow">
            <form method="GET" action="{{ route('parent.children.evaluations.historique.search', $student->id) }}" class="grid grid-cols-1 gap-3 sm:grid-cols-3">
                ...
                <div class="flex items-end gap-2">
                    ...
                    <a href="{{ route('parent.children.evaluations.historique', $student->id) }}" class="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium hover:bg-slate-50">Reset</a>
                </div>
            </form>
        </div>
                        ...
                        <tbody>
                            @foreach ($dayEvaluations as $evaluation)
                                <tr class="border-b hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4">{{ $evaluation->halaqa->nom_halaqa ?? '-' }}</td>
        ...
        @empty
            <div class="rounded-lg bg-white p-6 text-center text-gray-500 shadow">Aucune evaluation trouvee.</div>
        @endforelse
@endsection
