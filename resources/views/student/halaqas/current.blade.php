...
@section('title', 'Ma Halaqa Actuelle')

@section('content')
    <header class="mb-6 rounded-2xl bg-gradient-to-r from-[#04371f] to-[#04371f]/80 p-6 text-white shadow-xl">
        <h1 class="text-2xl font-bold">Ma halaqa actuelle</h1>
        <p class="mt-2 text-sm text-slate-200">Halaqa active actuelle et toutes vos evaluations.</p>
    </header>
    ...
    @elseif ($halaqas->isEmpty())
        <section class="rounded-xl bg-white p-6 shadow text-center">
            <p class="text-slate-600 text-lg">Aucune halaqa active pour le moment.</p>
            <p class="text-slate-500 text-sm mt-2">Veuillez contacter l'administration pour etre inscrit dans une halaqa.</p>
        </section>
    @else
        <section class="overflow-hidden rounded-xl bg-white shadow">
         ...
         <section class="mb-6 mt-6 rounded-xl bg-white p-4 shadow">
            <form method="GET" action="{{ route('student.halaqas.search', $halaqas->first()->id) }}" class="flex flex-col gap-3 sm:flex-row sm:items-end">
        ...
        <section class="mt-6">
            ...
            @forelse ($evaluationsByDate as $date => $dayEvaluations)
                <div class="mb-6 overflow-hidden rounded-xl bg-white shadow">
                    <div class="border-b border-slate-100 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-800">
                            ...
                            <tbody>
                                @foreach ($dayEvaluations as $evaluation)
                                    <tr class="border-t border-slate-100 align-top">
                                        <td class="px-4 py-3 whitespace-nowrap">{{ optional($evaluation->created_at)->format('H:i') ?? '-' }}</td>
            ...
            @empty
                @if (isset($search_results) && $search_results === true)
                    <div class="rounded-xl bg-white p-6 text-center text-slate-500 shadow">Aucun résultat trouvé pour votre recherche.</div>
                @else
                    <div class="rounded-xl bg-white p-6 text-center text-slate-500 shadow">Aucune evaluation pour votre halaqa active.</div>
                @endif
            @endforelse
        </section>
    @endif
