...
@section('content')
    <header class="mb-6 rounded-2xl bg-gradient-to-r from-[#04371f] to-[#04371f]/80 p-6 text-white shadow-xl flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold">Modifier competition #{{ $competition->id }}</h2>
            <p class="mt-2 text-sm text-slate-200">Mise a jour des informations de la competition.</p>
        </div>
    ...
    <section class="rounded-xl bg-white p-5 shadow xl:max-w-6xl">
        <form method="POST" action="{{ route('competitions.update', ['competition' => $competition->id]) }}" class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            @csrf
            @method('PUT')
            ...
            <div class="lg:col-span-7 grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div class="sm:col-span-2">
                    ...
                    <input id="titre" name="titre" type="text" required class="w-full rounded-lg border border-slate-300 px-3 py-2" value="{{ old('titre', $competition->titre) }}">
                </div>

                <div class="sm:col-span-2">
                    <label class="mb-1 block text-sm font-medium" for="description">Description</label>
                    <textarea id="description" name="description" rows="4" class="w-full rounded-lg border border-slate-300 px-3 py-2">{{ old('description', $competition->description) }}</textarea>
                </div>

                <div>
                    <label class="mb-1 block text-sm font-medium" for="date_debut">Date debut</label>
                    <input id="date_debut" name="date_debut" type="date" required class="w-full rounded-lg border border-slate-300 px-3 py-2" value="{{ old('date_debut', $competition->date_debut) }}">
                </div>

                <div>
                    <label class="mb-1 block text-sm font-medium" for="date_fin">Date fin</label>
                    <input id="date_fin" name="date_fin" type="date" required class="w-full rounded-lg border border-slate-300 px-3 py-2" value="{{ old('date_fin', $competition->date_fin) }}">
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <select id="statut" name="statut" required class="w-full rounded-lg border border-slate-300 px-3 py-2">
                        ...
                        @foreach (['active', 'inactive'] as $statut)
                            <option value="{{ $statut }}" @selected(old('statut', $competition->statut ?? '') === $statut)>
                                {{ ucfirst($statut) }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="lg:col-span-5 flex flex-col lg:pt-0">
                ...
                @php
                    ...
                    $allStudents = $students
                        ->merge($studentsNotInCompetition)
                        ->unique('id')
                        ->sortBy('id');
                @endphp
            ...
            <div class="lg:col-span-12 mt-2 pt-4 border-t border-slate-100 flex justify-end">
                <button type="submit" class="rounded-lg bg-blue-600 px-8 py-2.5 font-semibold text-white shadow-sm hover:bg-blue-500 transition-colors">Mettre a jour</button>
            </div>
        </form>
    </section>
@endsection
